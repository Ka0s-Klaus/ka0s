...
logger = logging.getLogger("ka0s.ingest_ops_telemetry")
...
def chunk_text(text: str, chunk_chars: int, overlap_chars: int = 0) -> List[str]:
    ...
    # Ops Telemetry Splitting: Use regex to keep stacktraces intact if possible
    # We prefer splitting by double newlines or timestamps
    separators = [
        "\n\n",
        "\n[0-9]{4}-[0-9]{2}-[0-9]{2}", # Timestamp YYYY-MM-DD
        "\n[IWEF][0-9]{4}", # Klog level
        "\n",
        " "
    ]
