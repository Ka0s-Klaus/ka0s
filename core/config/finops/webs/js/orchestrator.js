...
function initNavbar() {
    ...
    if (navbarTitle && window.webConfig && window.webConfig.title) {
        navbarTitle.textContent = window.webConfig.title;
    }

    // Cargar las secciones
    const navbar = document.getElementById('navbar-sections');
    if (navbar && window.webConfig && window.webConfig.sections && Array.isArray(window.webConfig.sections)) {
        window.webConfig.sections.forEach(section => {
...
async function handleHashChange() {
    ...
    if (hash && window.webConfig && window.webConfig.sections) {
        // Buscar la sección que coincide con el hash
        const section = window.webConfig.sections.find(s => 
