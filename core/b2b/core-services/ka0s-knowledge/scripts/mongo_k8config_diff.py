...
logger = logging.getLogger("ka0s.mongo_ingest")
...
def get_unified_diff(old_dict: Any, new_dict: Any) -> str:
    old_str = json.dumps(old_dict, ensure_ascii=False, default=str, indent=2, sort_keys=True).splitlines(keepends=True)
    new_str = json.dumps(new_dict, ensure_ascii=False, default=str, indent=2, sort_keys=True).splitlines(keepends=True)
    diff = difflib.unified_diff(old_str, new_str, fromfile='previous_state', tofile='current_state')
    return ''.join(diff)
...
def fetch_previous_doc(col: Collection, current_id: Any) -> Optional[Dict[str, Any]]:
    return col.find_one({"_id": {"$lt": current_id}}, sort=[("_id", -1)])


def diff_to_text(db_name: str, collection_name: str, current_doc: Dict[str, Any], prev_doc: Optional[Dict[str, Any]]) -> str:
    doc_id = str(current_doc.get("_id", ""))
    database_id = str(current_doc.get("databaseId", ""))
    workflow_db_id = str(current_doc.get("workflowDatabaseId", ""))
    
    header = f"source=mongo_diff db={db_name} collection={collection_name} id={doc_id}"
    if database_id:
        header += f" databaseId={database_id}"
    if workflow_db_id:
        header += f" workflowDatabaseId={workflow_db_id}"
    ...
    if prev_doc:
        prev_db_id = str(prev_doc.get("databaseId", ""))
        header += f" previousDatabaseId={prev_db_id}"
    header += "\n\n"
    
    current_data = current_doc.get("data", current_doc)
    prev_data = prev_doc.get("data", prev_doc) if prev_doc else {}
    
    # Remove _id for cleaner diff if it exists
    if isinstance(current_data, dict) and "_id" in current_data:
        current_data = {k: v for k, v in current_data.items() if k != "_id"}
    if isinstance(prev_data, dict) and "_id" in prev_data:
        prev_data = {k: v for k, v in prev_data.items() if k != "_id"}

    diff_str = get_unified_diff(prev_data, current_data)
    if not diff_str.strip():
        return header + "[NO CHANGES DETECTED]"
        
    return header + "[KUBERNETES CONFIGURATION DIFF]\n" + diff_str
...
def vectorize_collection(
    ...
    try:
        ...
        for doc in cursor:
            ...
            doc_id = str(doc.get("_id", ""))
            prev_doc = fetch_previous_doc(col, doc.get("_id"))
            
            # Text to vectorize: Diff
            diff_text = diff_to_text(db_name, collection_name, doc, prev_doc)
            
            # Combine both state (doc_to_text) and diff (diff_to_text) or just diff?
            # The user asked specifically for differences. It's best to index both,
            # but to ensure we answer "what changed", indexing the diff is crucial.
            # We'll index both the diff and the raw state for comprehensive RAG.
            state_text = doc_to_text(db_name, collection_name, doc)
            
            chunks_state = chunk_text(state_text, run_cfg.chunk_chars, run_cfg.chunk_overlap)
            chunks_diff = chunk_text(diff_text, run_cfg.chunk_chars, run_cfg.chunk_overlap)
            
            chunks = chunks_state + chunks_diff
            ...
            with pg_conn:
                for i, chunk in enumerate(chunks):
                    if not chunk.strip():
                        continue
                    source = f"mongo_diff://{db_name}/{collection_name}/{doc_id}#chunk{i}"
