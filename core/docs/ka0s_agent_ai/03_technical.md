...
2. **Cerebro (LLM):** Modelos locales (Ollama) como `llama3.2` que razonan utilizando el paradigma **ReAct** (Reason + Act).
...
2. El runner (`swarm-runners-scaleset`) se levanta y verifica que los modelos necesarios (`llama3.1` o `llama3.2`) estén disponibles en el pod de Ollama.
